...
async function supabaseRequest(table: string, method: string, body?: any, query?: any) {
  ...
  // POST 请求需要 return=representation 以获取插入的数据
  if (method === 'POST') {
...
export const tcb = {
  database: () => ({
    collection: (name: string) => ({
      where: (query: any) => {
        // 将 CloudBase 的 where 格式转换为 Supabase 的 eq 格式
        const supabaseQuery: any = {};
        if (query) {
          Object.entries(query).forEach(([k, v]) => {
            supabaseQuery[k] = `eq.${v}`;
          });
        }
        return {
          get: async () => supabaseRequest(name, 'GET', null, supabaseQuery),
          count: async () => {
            const { data } = await supabaseRequest(name, 'GET', null, { ...supabaseQuery, select: 'count' });
